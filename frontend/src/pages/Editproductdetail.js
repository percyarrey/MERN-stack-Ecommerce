...
function Editproductdetails() {
  ...
  //ROUTING
  const {id} = useParams()
  var newcartData = (productData.filter(item=>(item._id)===id))[0] || productData[0] ||0
  ...
  const [data,setdata]=React.useState({
    id:newcartData._id,
    productname:newcartData.productname,
    category:newcartData.category,
    image:newcartData.image,
    price:newcartData.price,
    description:newcartData.description
  })
    ...
    if(!saveSpinner){
      if(productname && image&& price && description){
        if(category!="none"){
          if(price>0){
            setSaveSpinner(true)
            const fetchData = await fetch(`${process.env.REACT_APP_SERVER_DOMAIN}/editproduct`,{
              method: "POST",
              headers:{
            ...
            const dataRes = await fetchData
                if(dataRes.message=="Product upload Failed"){
                  ...
                  if(dataRes.message=="Product Successfully Updated"){
                    console.log(dataRes)
...
return (
  <div className='w-100 d-flex justify-content-center align-items-center'>
      {
        userData.fname!==undefined && newcartData!==undefined?
        <div className=' shadow rounded-3' style={{maxWidth:"30rem",paddingLeft:"1rem",paddingRight:"1rem",paddingBottom:"1.2rem",backgroundColor:"white"}}>
            ...
            <div className='text-center mt-2 text-danger'>{warning}</div>
              ...
              {saveSpinner?<TailSpin color='white' width={50} height={28}/>:"Update product"}
              </button>
            </form>
        </div>:
